...
do
	./VM/corewar $@ -dump $i | sed -n -e '/^0x/p' > scripts/compare/fake
	./vm_champs/corewar $@ -d $i | sed -n -e'/^0x/p' > scripts/compare/real
	if ! cmp -s "scripts/compare/fake" "scripts/compare/real"; then
