...
/*   Updated: 2020/01/30 21:25:34 by krioliin      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */

#include "corewar_visual.h"

short	get_color(t_cursor *cursor)
{
	short id;
...
{
	wattron(warena, COLOR_PAIR(get_color(cursor)));
...
bool	cursor_moved(t_cursor *cursor, t_vm *vm, int current_x, int current_y)
{
	...
	{
		wattron(vm->v->warena, COLOR_PAIR(get_color(cursor)));
		...
		uint8_t arena[MEM_SIZE], t_vm *vm)
{
	...
	while (cursor)
	{
		...
		if (blink == false)
		{
			wattron(warena, COLOR_PAIR(get_color(cursor) * 10));
